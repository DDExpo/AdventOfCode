...
with open(Path(__file__).resolve().parent / "task_data", "r") as file:
    ...
    while pq:
        ...
        hp -= 1
        if hp <= 0 or mana_spent > answer: continue
        ...
        for spell, characteristics in SPELLS.items():
            newMn = mana - characteristics["cost"]
            if newMn < 0: continue
            ...
            if newHP > 0:
                heapq.heappush(pq, (spentMn, bHP, newMn, newHP, newShield, newPoison, newRecharge))

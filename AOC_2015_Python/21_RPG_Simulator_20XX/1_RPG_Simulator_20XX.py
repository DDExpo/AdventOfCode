...
with open(Path(__file__).resolve().parent / "task_data", "r") as file:
    ...
    def simulateBossFightWin(bossHP: int, heroHP: int, heroDamage: int, heroDefense: int, goldSpent: int) -> int:
        while heroHP > 0:
    ...
    for _, (cost, damage, _) in weapons.items():
        ...
        for _, (armor_cost, _, defense) in armors.items():
            ...
            for rings_taken in range(3):
                if rings_taken == 0:
                    answer = simulateBossFightWin(boss_hp, hero_hp, damage, defense, cur_gold_spent)
                    
                if rings_taken == 1:
                    for ring_cost, ring_damage, ring_defense in rings:
                        if cur_gold_spent + ring_cost > answer: continue
                        answer = simulateBossFightWin(boss_hp, hero_hp, damage+ring_damage, defense+ring_defense, cur_gold_spent + ring_cost)

                if rings_taken == 2:
                    for i in range(6):
                        for ii in range(i+1, 6):
                            if cur_gold_spent + rings[i][0] + rings[ii][0] > answer: continue
                            answer = simulateBossFightWin(
